...
import { ArrowLeftIcon, ChatTeardropDotsIcon, ChatTeardropTextIcon, CompassIcon, HeartIcon, MagnifyingGlassIcon, RankingIcon } from "@phosphor-icons/react";
...
export default function SideBar(){
    const navLinks = [
        ...
        {name: "Comments", to:"/comments", icon: <ChatTeardropTextIcon size={32}/>},
        {name: "Weekly", to:"/weekly", icon: <CalendarCheckIcon size={32}/>}

    ];
    return (
        <aside className="w-1/5 bg-[var(--primary)] text-black p-4 flex flex-col border-r-2 border-black">
            ...
            type="search"
            placeholder="Search..."
            className="w-full p-3 pr-10 rounded-full border-2 bg-white border-black drop-shadow-md  transition duration-200 focus:outline-none focus:ring-2 focus:ring-black"
            />
               <MagnifyingGlassIcon
                size={24}
                weight="regular"
            ...
            <nav className="space-y-2">
                {navLinks.map((link)=>(
                    ...
                    key={link.name}
                    to={link.to}
                    className="flex gap-x-2  text-2xl items-center group hover:font-semibold "
                    >
